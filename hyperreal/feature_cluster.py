...
class FeatureClustering(IndexPlugin):
    ...
    @atomic
    def split_cluster_into(self, cluster_id, split_into, iterations=10):
        """
        ...
        cluster, then refining that initial split for iterations. If the initial split
        generates more than split_into clusters, the smallest clusters will be merged
        together.
        ...
        if len(initial_split) > split_into:
            by_size = sorted(
                (len(split), tentative_id)
                for tentative_id, split in initial_split.items()
            )

            to_merge = by_size[: (-split_into + 1)]
            merge_into = to_merge[0][0]

            for _, tentative_id in to_merge:
                initial_split[merge_into] |= initial_split[tentative_id]
                del initial_split[tentative_id]

        splits = list(
            self._refine_clustering(
                initial_split, iterations=iterations, sampling_rate=0
            ).values()
        )

        # We'll leave the first lucky cluster in place instead of creating a new one -
        # this just means some of the features won't move.
