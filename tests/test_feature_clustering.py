...
def test_plaintext_feature_cluster(tmp_path):
    ...
    with cf.ProcessPoolExecutor(mp_context=mp.get_context("spawn")) as pool:
        ...
        clustering = moby_idx.plugins["feature_clusters"]
        ...
        # Merge clusters:
        clustering.merge_clusters([1, 0])
        assert len(clustering.cluster_ids) == 15
        assert (
            set(clustering.cluster_features(1)) == new_clustering[0] | new_clustering[1]
        )
